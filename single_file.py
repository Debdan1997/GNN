...
def genetic_algorithm(weight_list):
    ...
    for generation in range(generations):
        ...
        t1.start()
        fitness_list = []
        left = weight_list.qsize()
        while left>0:  ### while queue is not empty
            time.sleep(.001)
...
#test = [[25,1,1,1,1],[50,2,2,2,2],[5,3,3,3,3],[1,4,4,4,4]]
#q = create_new_generation(test)
#while not q.empty():
#    print(q.get())
#print(np.random.choice([1,2,3,4,5], 1, p=[0.1, 0, 0.3, 0.6, 0]))
